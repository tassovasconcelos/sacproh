alter table public.commercial_trial_requests
  add column if not exists campaign_code text not null default 'ORGANIC',
  add column if not exists lead_source text not null default 'LANDING_PAGE';

create index if not exists idx_commercial_trials_campaign
  on public.commercial_trial_requests(campaign_code, created_at desc);
...
begin
  ...
  values(request_row.company_name,request_row.company_name,normalized_document,jsonb_build_object('commercial_trial_request_id',request_row.id,'trial_user_limit',1,'campaign_code',request_row.campaign_code))
  ...
  values(new_tenant_id,selected_plan.id,'TRIAL',1,now()+interval '15 days',now(),now()+interval '15 days',request_row.work_email);
  update public.commercial_trial_requests set provisioned_tenant_id=new_tenant_id,provisioned_at=now(),status='TRIAL_ACTIVE',
    trial_starts_at=coalesce(trial_starts_at,now()),trial_ends_at=coalesce(trial_ends_at,now()+interval '15 days'),assigned_to=coalesce(assigned_to,p_actor_id),updated_at=now()
  where id=request_row.id;
  return jsonb_build_object('tenantId',new_tenant_id,'planCode',selected_code,'trialDays',15,'seatLimit',1,'alreadyProvisioned',false);
end $$;
